...
const fakeLog = { child: () => fakeLog, info() {}, debug() {}, warn() {}, error() {} } as unknown as RequestContext['log'];
...
describe('resolveRevokeMembership', () => {
  it('throws ForbiddenError for STAFF', async () => {
    await expect(
      resolveRevokeMembership({}, { membershipId: '00000000-0000-0000-0000-000000000001' }, ctxFor({ ...ownerAuth, role: 'STAFF' })),
    ).rejects.toBeInstanceOf(ForbiddenError);
  });

  it('throws NotFoundError when membership belongs to another tenant', async () => {
    mockMembershipFindUnique.mockResolvedValueOnce({ id: 'm-x', tenantId: 't-other', role: 'STAFF', status: 'ACTIVE' });
    await expect(
      resolveRevokeMembership({}, { membershipId: '00000000-0000-0000-0000-000000000001' }, ctxFor(ownerAuth)),
    ).rejects.toBeInstanceOf(NotFoundError);
  });

  it('refuses to revoke the last active OWNER', async () => {
    mockMembershipFindUnique.mockResolvedValueOnce({ id: 'm-1', tenantId: 't-1', role: 'OWNER', status: 'ACTIVE' });
  ...
  it('revokes a non-owner membership and writes audit', async () => {
    mockMembershipFindUnique.mockResolvedValueOnce({ id: 'm-1', tenantId: 't-1', role: 'STAFF', status: 'ACTIVE' });
