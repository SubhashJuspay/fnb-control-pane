...
describe('resolveUpdateMembershipRole', () => {
  it('throws ForbiddenError for STAFF', async () => {
    await expect(
      resolveUpdateMembershipRole({}, { membershipId: '00000000-0000-0000-0000-000000000001', role: 'ADMIN' }, ctxFor({ ...ownerAuth, role: 'STAFF' })),
    ).rejects.toBeInstanceOf(ForbiddenError);
  });

  it('throws NotFoundError when membership is in another tenant', async () => {
    mockMembershipFindUnique.mockResolvedValueOnce({ id: 'm-1', tenantId: 't-other', role: 'STAFF' });
    await expect(
      resolveUpdateMembershipRole({}, { membershipId: '00000000-0000-0000-0000-000000000001', role: 'MANAGER' }, ctxFor(ownerAuth)),
  ...
  it('refuses to demote the last active OWNER', async () => {
    ...
    await expect(
      resolveUpdateMembershipRole({}, { membershipId: '00000000-0000-0000-0000-000000000001', role: 'ADMIN' }, ctxFor(ownerAuth)),
